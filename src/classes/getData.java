...
public abstract class getData {
    ...
    public static int countMonthlyBills(File file) throws FileNotFoundException{
        int count = 0;
        try(Scanner input = new Scanner(file)){
            while(input.hasNext()){
                String test = input.next();
                if(test.matches("readingMeter[\\d]"+":") || test.matches("readingMeter[\\d][\\d]"+":" )|| test.matches("readingMeter[\\d][\\d][\\d]"+":") )
                    count++;
            }
        }
        return count;
